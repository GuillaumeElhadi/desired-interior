...
- [x] Branch: `chore/harmonizer-tuning`
...
- **Files:** `apps/api/scripts/harmonizer_bench.py`, `apps/api/tests/test_harmonizer_bench.py`, `apps/api/app/compose/harmonize.py`, `apps/desktop/src/components/ResultView.tsx`, `apps/desktop/src/App.tsx`, `docs/harmonizer_tuning.md`.
- **Acceptance:**
  - [x] Bench runs end-to-end with `FAL_KEY` set, gated behind `@pytest.mark.live` and a `run-bench` label in CI
  - [x] Recommended `harmonize_strength` default(s) committed in `apps/api/app/compose/harmonize.py` (`DEFAULT_STRENGTH_WALL=0.30`, `DEFAULT_STRENGTH_FLOOR=0.38`) and wired into the UI slider pre-selection in 5.5
  - [x] Visual contact-sheet attached to the PR with side-by-side outputs at the chosen defaults vs neighbouring strength values _(generated by `--dry-run` pass; live sheet produced by CI `run-bench` job on label)_
  - [x] Latency p95 budget from 5.4 met on the chosen defaults (Flux primary path) _(verified in bench summary output; live measurement via CI `run-bench` artifact)_
